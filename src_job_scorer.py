...
class JobScorer:
    ...
    def _calculate_keyword_boost(self, job: Job) -> float:
        ...
        for keyword, weight in self.keyword_boosts.items():
    ...
    def _find_keyword_matches(self, job: Job) -> List[str]:
        """Find matching keywords in job posting"""
        job_text = f"{job.title} {job.description}".lower()
        matches = []
        
        for keyword in self.keyword_boosts.keys():
            if keyword.lower() in job_text:
    ...
    def _check_language_fit(self, job: Job) -> bool:
        """Check if job language requirements match"""
        job_text = job.description.lower()
        
        # Check for English requirement
        if "english" in job_text or "english fluent" in job_text:
            return "English" in self.profile.working_languages
        
        # Check for Thai requirement
        if "thai fluent" in job_text or "native thai" in job_text:
            return False  # User is not fluent in Thai
        
        return True
    
    def _check_location_fit(self, job: Job) -> bool:
        """Check if location fits user constraints"""
        location_lower = job.location.lower()
        ...
        for country in self.profile.allowed_countries:
            if country.lower() in location_lower or country.lower() == "remote":
                return True
    ...
    def score_batch(self, jobs: List[Job]) -> List[JobAssessment]:
        """Score multiple jobs"""
        assessments = [self.assess_job(job) for job in jobs]
        return sorted(assessments, key=lambda x: (-x.score, x.job.title))